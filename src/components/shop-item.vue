    ...
    <div>
        ...
        <div class="params-colors">
            <ul class="colors-list">
                <li v-for= "(aitem,i) in item.sku_info" v-bind:key="i"><a href="javascript:;"  @click="tabIndex(i)" :class="{'active':itemIndex===i}" :title="aitem.spec_json.show_name"><img :src="'http://img01.smartisanos.cn/'+aitem.spec_json.image+'20X20.jpg'"></a></li>
            </ul>
        </div>
        <div class="item-btns clearfix">
            <span class="item-gray-btn"><router-link :to="{path:'item',name:'Item',query:{itemId:item.sku_info[itemIndex].sku_id}}">查看详情</router-link></span>
            <span class="item-blue-btn" @click="addCarPanelHandel(item.sku_info[itemIndex])">加入购物车 </span>
        </div>
        ...
        <div class="item-cover">
          <router-link :to="{path:'item',name:'Item',query:{itemId:item.sku_info[itemIndex].sku_id}}"/>
        </div>
    </div>
  </div>
</template>
