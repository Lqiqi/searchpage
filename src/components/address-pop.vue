                                ...
                                <div class="address-form">
                                    ...
                                    <div class="module-form-row">
                                        <div class="form-item-v3" :class="{'form-invalid-item':phoneError}">
                                            ...
                                            <input type="text" class="js-verify" v-model="receive.phone" @blur="verificPhone" @focus="setverificPhone">
                                            <div class="verify-error"></div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="module-form-row clear">
                                        <div class="form-item-v3 select-item province-wrapper">
                                            <select name="province_code" class="province select-province js-form-province js-verify" v-model="receive.provinceId">
                                                ...
                                                <option :value="item.area_id" v-for="(item,index) in addList" :key="index">{{item.area_name}}</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="module-form-row clear">
                                        <div class="form-item-v3 select-item city-wrapper fn-left form-focus-item">
                                            <select class="city select-city js-form-city js-verify" v-model="receive.cityId">
                                                ...
                                                <option :value="item.area_id" v-for="(item,index) in cityList" :key="index">{{item.area_name}}</option>
                                            </select>
                                        </div>
                                    ...
                                    <div class="module-form-row">
                                        <div class="form-item-v3">
                                            <i v-if="!receive.add">详细地址，如街道名称，楼层，门牌号码等</i>
                                    ...
                                    <div class="module-form-row fn-clear">
                                        ...
                                        <span class="blue-checkbox" :class="{'blue-checkbox-on':receive.default}" @click="defaultreceie"></span>设为默认
                                    </div>
...
export default {
  ...
  created() {
    this.tooldReceive = JSON.parse(JSON.stringify(this.oldReceive))
    this.defaultReceive = JSON.parse(JSON.stringify(this.receive))
    console.log(this.defaultReceive)
    if (this.tooldReceive) {
      console.log(this.tooldReceive, 'tooldReceive')
      this.recevieIndex = this.oldrecevieIndex
      this.receive = JSON.parse(JSON.stringify(this.tooldReceive))
  ...
  watch: {
    'receive.provinceId'() {
      ...
      if (this.receive.provinceId != 0) {
        let province = this.addList.filter(item => {
          return item.area_id == this.receive.provinceId
        })[0]
        this.cityList = province.city_list
        this.receive.province = province.area_name
      } else {
      ...
      if (this.tooldReceive) {
        this.receive.cityId = this.tooldReceive.cityId
        this.receive.city = this.tooldReceive.city
      }
    },
    'receive.cityId'() {
      this.receive.countyId = 0
      ...
      if (this.receive.cityId != 0) {
        let city = this.cityList.filter(item => {
          return item.area_id == this.receive.cityId
        })[0]
      ...
      if (this.tooldReceive) {
        this.receive.countyId = this.tooldReceive.countyId
        this.receive.county = this.tooldReceive.county
      }
    },
    'receive.countyId'() {
      if (this.receive.countyId != 0) {
        let county = this.countyList.filter(item => {
          return item.area_id == this.receive.countyId
        })[0]
  ...
  methods: {
    closePop() {
      this.$emit('close')
      this.receive = JSON.parse(JSON.stringify(this.defaultReceive))
      this.tooldReceive = null
    },
    openPop() {
      this.$emit('open')
    ...
    submitHandle() {
      let data = {
        index: this.recevieIndex,
        receive: this.receive
      }
      if (this.isRight) {
        this.$store.commit('submitReceive', data)
        this.receive = JSON.parse(JSON.stringify(this.defaultReceive))
