            ...
            <div class="cart-box">
                ...
                <div class="cart-inner">
                    <div class="empty-label" v-if="count<=0">
                        <h3>您的购物车中还没有商品</h3>
                        <router-link :to="{name:'Shop',path:'shop'}" class="link">现在选购</router-link>
                    </div>
                    <div v-else>
                                ...
                                <div class="cart-top-items" v-for="(item,index) in carPanelData" :key="index">
                                    <div class="cart-items">
                                        <div class="items-choose">
                                            <span class="blue-checkbox-new" :class="{'checkbox-on':item.checked}" @click="checkedItemhandle(item.sku_id)"><a></a></span>
                                        </div>
                                        ...
                                        <div class="item-cols-num">
                                            <div class="select js-select-quantity">
                                                <span class="down" :class="{'down-disabled':item.count<=1}" @click="subNumHandle(item.sku_id)">-</span>
                                                <span class="num">
                                                ...
                                                <span class="up" :class="{'up-disabled':item.count>=item.limit_num}" @click="addNumHandle(item.sku_id)">+</span>
                                            </div>
                                        </div>
                    ...
                    <div class="cart-bar-operation">
                        <div>
                            <div class="choose-all js-choose-all">
                                <span class="blue-checkbox-new" :class="{'checkbox-on':allchecked}" @click="allCheckHandle(allchecked)"><a></a></span>
                                全选
                            </div>
                            <div class="delete-choose-goods" @click="deletechecked()">删除选中的商品</div>
                        </div>
                    </div>
                    <div class="shipping">
                        ...
                        <span class="jianguo-blue-main-btn big-main-btn js-checkout" :class="{'disabled-btn':checkedlen<=0}"><router-link to="checkout">现在结算</router-link></span>
                    </div>
                </div>
            </div>
        </div>
        <prompt v-if="maxOff"></prompt>
    </div>
</template>
<script>
...
export default{
  ...
  computed: {
    ...
    allchecked() {
      const allchecked = this.carPanelData.every((ele) => {
    ...
    checkedlen() {
      ...
      checkeditem = this.carPanelData.filter((ele) => {
        return ele.checked === true
      }).reduce((pre, cur) => {
  ...
  methods: {
    ...
    deletechecked() {
      let i = this.carPanelData.length
      while (i--) {
        if (this.carPanelData[i].checked) {
          this.$delete(this.carPanelData, i)
        }
      }
    },
    checkedItemhandle(id) {
      const checkedItem = this.carPanelData.filter((ele) => {
        return ele.sku_id === id
      })[0]
      checkedItem.checked = !checkedItem.checked
    },
    allCheckHandle(allchecked) {
      let allItem = null
      allItem = !allchecked
      this.carPanelData.forEach(ele => {
        ele.checked = allItem
      })
    }
  }
