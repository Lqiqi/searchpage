  ...
  <div class="content page-order-payment">
      <div class="gray-box clear">
          ...
          <div class="box-inner payment-checkout-panel clear">
              <span class="jianguo-blue-main-btn big-main-btn js-payment-order" @click="payNowpayHandle(ordermsg.orderId)">
                  <a>现在支付</a>
              </span>
              <span class="prices"> 应付金额：   <em><span>¥ </span>{{ordermsg.checkedprice+ordermsg.frheight}}</em>   </span>
          </div>
      </div>
      <div class="confirm-detail">
          <div class="info-title">订单编号</div>
          <p class="info-detail">{{ordermsg.orderId}}</p>
      </div>
      <div class="confirm-detail">
          <div class="info-title">收货信息</div>
          <p class="info-detail">姓名：{{ordermsg.receiveInfo.name}}</p>
          <p class="info-detail">联系电话：{{ordermsg.receiveInfo.phone}}</p>  <p class="info-detail">详细地址：{{ordermsg.receiveInfo.province}}{{ordermsg.receiveInfo.city}}{{ordermsg.receiveInfo.county}}{{ordermsg.receiveInfo.add}}</p>
      </div>
      <div class="confirm-detail">
          <div class="info-title">发票信息</div>
          <p class="info-detail">发票类型：电子发票</p>
          <p class="info-detail">发票抬头：{{ordermsg.receiptInfoper}}</p>
      ...
      <div class="confirm-goods-table">
          <div class="cart-items clear" v-for="(item,index) in ordermsg.goodsData" :key="index">
      ...
      <div class="order-discount-line">
          <p> 商品总计： <span>¥ {{ordermsg.checkedprice+ordermsg.frheight}}</span> </p>
          <p> 运费： <span>+ ¥ {{ordermsg.frheight}}</span> </p>
      </div>
  </div>
</div>
</template>
...
export default {
  computed: {
    ordermsg() {
      ...
      let ordermsg = orderList.filter(item => {
  ...
  methods: {
    payNowpayHandle(id) {
      ...
      alert('已完成支付' + (this.ordermsg.checkedprice + this.ordermsg.frheight) + '元')
      this.$router.push({ name: 'Account', path: '/account' })
    }
  }
}
</script>
