          ...
          <div class="box-inner js-checkout-address-panel ">
            <div class="address-common-table js-multiple-address-panel">
              <ul class="address-item-list clear js-address-item-list">
                <li class="js-choose-address "  :class="{'selected-address-item':receiveIndex===index}"  v-for="(item,index) in receiveInfo" :key="index" @click="changereceive(index)">
      ...
      <div class="gray-box">
        ...
        <div class="box-inner invoice-box js-invoice-box">
          ...
          <div class="invoice-detail"> 发票抬头：
            <div class="radio-box">
              <label  @click="checkreceipt(true)">
                <input type="radio" class="hide">
                <span class="blue-radio" :class="{'blue-radio-on':receiptInfo.personal}"><a></a></span>  个人
              </label>
              <label  @click="checkreceipt(false)">
                <input type="radio" class="hide">
                <span class="blue-radio" :class="{'blue-radio-on':!receiptInfo.personal}"><a></a></span>  单位
              </label>
            </div>
            <div class="module-form-row form-item fn-hide js-invoice-title" v-if="!receiptInfo.personal">
              <div  class="module-form-item-wrapper no-icon small-item" >
                <i v-if="!receiptInfo.name">请填写公司发票抬头</i>
                <input type="text" class="js-verify" v-model="receiptInfo.name">
              </div>
            </div>
          </div>
      ...
      <div class="gray-box">
        ...
        <div class="box-inner ui-goods-cart">
          ...
          <div class="cart-table">
            <div class="cart-group js-cart-group">
              <div class="cart-items" v-for="(item,index) in checkedItem" :key="index">
        ...
        <div class="box-inner">
          <div class="order-discount-line">
            ...
            <p> 运费： <span>+ ¥ {{frheight}} </span> </p>
            <p class="discount-line js-discount-cash"> <em>现金券</em>： <span> - 0 </span> </p>
          </div>
        </div>
        <div class="box-inner">
          <div class="last-payment clear">
            <span class="jianguo-blue-main-btn big-main-btn payment-blue-bt fn-right js-checkout" @click="submitOrderHandle"> <a>提交订单</a> </span> <span class="prices fn-right">应付金额： <em>¥ {{frheight+checkedprice}}</em></span>
          </div>
        </div>
      </div>
    </div>
...
export default{
  ...
  methods: {
    changereceive(i) {
      this.receiveIndex = i
      this.$set(this.receiveInfo[i], 'checked', true)
    },
    closePop() {
      this.popShow = false
      this.receiveInfo.forEach((receive, index) => {
    ...
    checkreceipt(bool) {
      this.receiptInfo.personal = bool
    },
    submitOrderHandle() {
      if (!this.receiptInfo.personal && !this.receiptInfo.name) return
      if (this.receiptInfo.personal) {
        this.receiptInfo.name = '个人'
      }

      let ordermsg = this.receiveInfo[this.receiveIndex]
      let iDate = new Date()
      ...
      let orderData = {
        ...
        goodsData: this.checkedItem,
        receiveInfo: ordermsg,
        receiptInfoper: this.receiptInfo.name,
        checkedprice: this.checkedprice,
        frheight: this.frheight,
        iDate: year + '-' + month + '-' + day,
        isPay: false
      }
      this.$store.commit('submitOrderfn', orderData)
      this.$router.push({
        name: 'Payment',
        path: 'payment',
