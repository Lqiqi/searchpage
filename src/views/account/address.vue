    ...
    <div class="account-address">
      <div class="gray-box clear">
        <div class="title pre-title">
          ...
          <span class="gray-blue-btn js-add-address" @click="openPop">
            <a class="add">添加新地址</a>
          </span>
        </div>
        <div class="box-inner clear" v-if="addressData.length" >
          <div class="address-list-item default-item"  :data-id="new Date().getTime()" v-for="(item,index) in addressData" :key="index">
            ...
            <div class="operation fn-right">
              <div class="operation-cell">
                <a class="gray-edit-btn js-edit-address" @click="editReceive(item,index)">修改</a>
                <span class="red-del-btn js-del-address" @click="deleteAddressHandle(item)">
                  <a>删除</a>
                  <em>删除</em>
                </span>
              </div>
         ...
         <div class="address-form clear" v-else>暂无订单信息</div>
      </div>
    </div>
    <address-pop v-if="popShow" @close="closePop()" :oldReceive="oldReceive" :oldrecevieIndex="recevieIndex" ></address-pop>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    checkDefaultHandle(item) {
      this.$store.commit('checkDefault', item)
    },
    deleteAddressHandle(item) {
      this.$store.commit('deleteAddress', item)
    },
    editReceive(item, index) {
      this.popShow = true
      this.oldReceive = item
      this.recevieIndex = index
    },
    openPop() {
      this.popShow = true
