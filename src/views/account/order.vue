    ...
    <div class="account-order">
      <div class="gray-box">
        ...
        <div class="js-list-inner">
          <div class="box-inner order-cart order-list-cart clear" v-for="(orderItem,index) in orderData" :key="index">
            <div class="gray-sub-title cart-title">
              <span class="date">{{orderItem.iDate}}</span>
              <span class="order-id"> 订单号： <a href="javascript:;">{{orderItem.orderId}}</a> </span>
              <router-link class="order-detail" tag="span" :to="{name:'Payment',path:'payment',query:{orderId:orderItem.orderId}}"><a href="javascript:;">查看详情&gt;</a> </router-link>
               <span class="sub-total">应付总额</span>
              <span class="num">数量</span>
              <span class="price">单价</span>
            </div>
            <div class="cart">
              <div class="cart-items clear" v-for="(item,index) in orderItem.goodsData" :key="index">
                <div class="prod-info clear">
                  ...
                  <div class="items-params clear">
                    <div class="name vh-center">
                      <a href="javascript:;" target="_blank" :title="item.sub_title+'（'+item.spec_json.show_name+'）'">{{item.sub_title}}（{{item.spec_json.show_name}}）</a>
                    </div>
            ...
            <div class="prod-operation">
              <div class="total">¥ {{orderItem.checkedprice+orderItem.frheight}}</div>
              <div class="status"><router-link :to="{name:'Payment',path:'payment',query:{orderId:orderItem.orderId}}" class="blue-small-btn js-payment-order"  v-if="!orderItem.isPay">现在付款</router-link><span v-else>已完成</span></div>
            </div>
          </div>
        </div>
      </div>
